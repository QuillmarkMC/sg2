...
tellraw @a [{"translate":"text.lobby.start.cancel","color":"red","bold": true,"clickEvent": {"action":"run_command","value": "/function sg:lobby/start_logic/interrupt/check_permission"},"hoverEvent":{"action":"show_text","contents":[{"translate":"text.lobby.start.cancel.hover","italic":true,"color":"gray"}]}}]
