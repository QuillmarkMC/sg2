...
execute if score $MaxTeamSize options matches 1 if score $ReadyNum lobbyVar >= $MAX_PLAYERS var run tellraw @s [{"text":"[!] ","color":"dark_red","bold": true},{"translate":"sg.lobby.start.warn.max_players","color":"white","bold": false,"with": [{"score": {"name": "$MAX_PLAYERS","objective": "var"}}]}]
#teams
execute unless score $MaxTeamSize options matches 1 if score $ReadyNum lobbyVar >= $MAX_PLAYERS var run tellraw @s [{"text":"[!] ","color":"dark_red","bold": true},{"translate":"sg.lobby.start.warn.max_players","color":"white","bold": false,"with": [{"score": {"name": "$MAX_PLAYERS","objective": "var"}}]}]
